# with some hacks for collision purposes
"""
...
class ObjMaterial:
    ...
    def getEggTexture(self):
        ...
        m.setMagfilter(EggTexture.FT_nearest)
        m.setMinfilter(EggTexture.FT_nearest_mipmap_linear)
...
class MtlFile:
    ...
    def __init__(self, filename=None):
        ...
        if filename is not None:
            self.path = "".join(os.path.split(filename)[:-1])
            self.read(filename)
    ...
    def read(self, filename, verbose=False):
        ...
        for line in file.readlines():
            ...
            if tokens[0] in ("map_Kd", "map_Bump", "map_Ks", "map_bump", "bump"):
                ...
                # map_Bump == bump
                # map_Ks == specular
                mat_file_name = tokens[1]
                if self.path:
                    mat_file_name = self.path + "/" + mat_file_name

                mat.put(tokens[0], pathify(mat_file_name))
...
class ObjFile:
    ...
    def read(self, filename, verbose=False):
        ...
        for line in file.readlines():
            ...
            if not tokens:
                continue
            if tokens[0] == "mtllib":
                mtllib_file = tokens[1:]
                if self.path:
                    mtllib_file = self.path + "/" + "".join(tokens[1:])
                else:
                    mtllib_file = tokens[1:]
                if True: print("mtllib:", mtllib_file)
                mtllib = MtlFile(mtllib_file)
    ...
    def __newobject(self, object):
        ...
        self.__closeobject()
        if True: print("__newobject:", "object:", object)
    ...
    def toEgg(self, verbose=True):
        ...
        # set the coordinate system to Y-up
        egg.setCoordinateSystem(2)
        ...
        for objname in self.objects:
            ...
            eobj.setCsType(196608)
            eobj.setCollideFlags(5242880)
